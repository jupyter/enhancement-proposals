...
- _Kernels that require encryption._ The proposal is backward-compatible: a kernel starts unencrypted when no keys are provided. In future some kernels could invert this and refuse to start without keys. There are no immediate plans to make that the default for kernels maintained by Project Jupyter, but it may suit kernel wrappers built for environments with sensitive data.
